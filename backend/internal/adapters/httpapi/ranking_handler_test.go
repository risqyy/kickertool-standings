...
func TestPublicRankingYearContractAndValidation(t *testing.T) {
	...
	reader := periodRankingReader{
		byYear:         map[int][]domain.PlayerAggregate{2025: {{PlayerName: "Player One", TotalPointsCents: &points}}},
	...
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"selectedYear":2025`) || !strings.Contains(response.Body.String(), `"availableYears":[2026,2025]`) || !strings.Contains(response.Body.String(), `"name":"Player One"`) {
