...
function TournamentCard({ item, selected, onSelect, onToggle }: { item: Tournament; selected: boolean; onSelect: () => void; onToggle: (included: boolean) => void }) {
  return <article className="rounded-md border bg-card p-4"><div className="flex items-start justify-between gap-3"><label className="flex items-center gap-3 font-medium"><Checkbox aria-label={item.name + ' auswählen'} checked={selected} onChange={onSelect} />{item.name}</label><Badge variant={item.includedInRanking ? 'success' : 'secondary'}>{item.includedInRanking ? 'Im Ranking' : 'Ausgeschlossen'}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-muted-foreground">Datum</dt><dd className="tabular">{formatDate(item.date)}</dd></div><div><dt className="text-muted-foreground">Status</dt><dd>{item.status || 'unbekannt'}</dd></div><div><dt className="text-muted-foreground">Standings</dt><dd><StandingStatus item={item} /></dd></div><div><dt className="text-muted-foreground">Quelle</dt><dd>{item.source}</dd></div></dl><div className="mt-4 flex items-center justify-between border-t pt-3"><span className="text-sm">Ranking-Beitrag</span><Switch aria-label={item.name + ' im Ranking'} checked={item.includedInRanking} onChange={event => onToggle(event.target.checked)} /></div></article>
}
