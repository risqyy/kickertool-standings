...
import { CheckCircle2, History, LoaderCircle, Search, ShieldAlert, Undo2, UserPlus } from 'lucide-react'
...
export function ManualCorrectionsPage() {
  ...
  const requestedPlayerId = Number(new URLSearchParams(window.location.search).get('playerId'))
  const [query, setQuery] = useState(() => new URLSearchParams(window.location.search).get('name') ?? '')
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (query.trim().length < 2) { setResults([]); setSearching(false); setSearchComplete(false); setSearchError(''); return }
      setSearching(true)
      setSearchComplete(false)
      setSearchError('')
      searchPlayers(query).then(values => {
        setResults(values)
        setActiveResultIndex(0)
        const requested = requestedPlayerId > 0 ? values.find(value => value.id === requestedPlayerId) : undefined
        if (requested) void choose(requested)
      }).catch(() => { setResults([]); setSearchError('Spielersuche konnte nicht geladen werden. Bitte versuche es erneut.') }).finally(() => { setSearching(false); setSearchComplete(true) })
    }, 250)
  ...
  return <div>
    ...
    <Card><CardHeader><CardTitle>Spieler auswählen</CardTitle><CardDescription>Suche umfasst kanonische Namen und Namensvarianten.</CardDescription></CardHeader><CardContent><div className="relative max-w-xl"><label htmlFor="correction-player-search" className="sr-only">Spieler suchen</label><Search className="absolute left-3 top-3.5 text-muted-foreground" size={17} aria-hidden="true" /><Input id="correction-player-search" className="pl-10" value={query} onChange={event => { setQuery(event.target.value); setPlayer(null); setActiveResultIndex(0); setSearchError('') }} onKeyDown={event => { if (results.length === 0) return; if (event.key === 'ArrowDown') { event.preventDefault(); setActiveResultIndex(index => Math.min(index + 1, results.length - 1)) } else if (event.key === 'ArrowUp') { event.preventDefault(); setActiveResultIndex(index => Math.max(index - 1, 0)) } else if (event.key === 'Enter') { event.preventDefault(); void choose(results[activeResultIndex]) } else if (event.key === 'Escape') { setResults([]) } }} placeholder="Mindestens 2 Zeichen" role="combobox" aria-autocomplete="list" aria-expanded={results.length > 0} aria-controls="correction-player-options" aria-activedescendant={results[activeResultIndex] ? `correction-player-option-${results[activeResultIndex].id}` : undefined} />{results.length > 0 && <div id="correction-player-options" className="absolute z-10 mt-2 w-full rounded-md border bg-card p-1 shadow-lg" role="listbox" aria-label="Spielergebnisse">{results.map((value, index) => <button type="button" role="option" id={`correction-player-option-${value.id}`} aria-selected={index === activeResultIndex} key={value.id} className={`flex min-h-11 w-full items-center justify-between rounded px-3 text-left text-sm hover:bg-muted ${index === activeResultIndex ? 'bg-muted' : ''}`} onClick={() => choose(value)}><span className="font-medium">{value.displayName}</span><span className="text-xs text-muted-foreground">{value.tournamentCount} Turniere</span></button>)}</div>}</div>{searching && <p className="mt-3 flex items-center gap-2 text-sm text-muted-foreground" role="status"><LoaderCircle size={16} className="animate-spin" aria-hidden="true" />Spieler werden gesucht …</p>}{searchError && <p className="mt-3 text-sm text-red-700" role="alert">{searchError}</p>}{!player && !searching && searchComplete && !searchError && query.trim().length >= 2 && results.length === 0 && <div className="mt-3 flex flex-wrap items-center gap-3"><p className="text-sm text-muted-foreground">Keine passenden Spieler gefunden.</p><Link className="inline-flex min-h-11 items-center gap-2 rounded-md border border-input bg-background px-3 text-sm font-medium hover:bg-accent" to={`/admin/players/new?name=${encodeURIComponent(query.trim())}`}><UserPlus size={16} aria-hidden="true" />Spieler anlegen</Link></div>}{player && <p className="mt-3 text-sm text-muted-foreground">Ausgewählt: <strong className="text-foreground">{player.displayName}</strong> · Korrekturversion {version}</p>}</CardContent></Card>
