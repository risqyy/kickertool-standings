...
- `frontend/` â€” one React + TypeScript + Vite + Tailwind/shadcn-style app, shared public/admin router, lazy admin routes, API client and UI components.
- `backend/` â€” one Go module with `cmd/`, `internal/`, domain/application/ports, API/HTML sources, GORM repository, scheduler and JSON HTTP handlers.
- `frontend/nginx.conf` â€” static SPA serving and `/api/` reverse proxy.
- `docker-compose.yml` â€” backend with SQLite volume and frontend Nginx service.
- `backend/api/openapi.yaml` â€” versioned JSON contract.
